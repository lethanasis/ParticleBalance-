...
def compute_te_mtx_element(args):
    j, i, pn_value, n_Ne_value, INTD, INTNe, fre_params, volume = args
    try:
        ions = IonHandler()
        ...
        Te = bisection(power_balance, a, b, ions, pn_value, Te, Tn, fre, Di, dist, NRE)
        Tn = Te * e
        ne, n = ionrate.equilibriumAtPressure(ions, pn_value, Te, Tn, fre)
        ions.setSolution(n)

        Z = Zeff(ions, ne)
        n, ne, Te, k = newton_method(ions, pn_value, ne, Te, fre, Z, Di, dist, NRE)
        if Te < 0:
            return j, i, -1  # Return -1 for invalid Te
        return j, i, Te
...
def main():
    ...
    args = [(j, i, pn[i], n_Ne[j], INTD, INTNe, fre_params, Volume) for j in range(n_Ne.size) for i in range(pn.size)]
    ...
    Te_mtx = np.zeros((n_Ne.size, pn.size))

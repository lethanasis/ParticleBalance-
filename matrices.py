...
def construct_matrix(ions: IonHandler, ne, Te, Z, fre=None, V_plasma=1, V_vessel=1):
    ...
    for ion in ions:
        ...
        for j in range(ion.Z+1):
            ...
            if j > 0 and j < ion.Z :
                A[off+j, N-3] += ne*(dIdne(j-1)*ion.solution[j-1] - dIdne(j)*ion.solution[j] + dRdne(j+1)*ion.solution[j+1] - dRdne(j)*ion.solution[j]) + I(j-1) * ion.solution[j-1] - I(j) * ion.solution[j] + R(j)*ion.solution[j] - R(j+1)*ion.solution[j]
                A[off+j, N-2] += dIdT(j-1)*ne*ion.solution[j-1] - dIdT(j)*ne*ion.solution[j] +dRdT(j+1)*ion.solution[j+1] -dRdT(j)*ion.solution[j]
            elif j==0 :
                A[off+j, N-3] += - ne*(dIdne(j)*ion.solution[j] + dRdne(j+1)*ion.solution[j+1] - dRdne(j)*ion.solution[j]) - I(j)*ion.solution[j+1] +R(j)*ion.solution[j+1]- R(j)*ion.solution[j]
                A[off+j, N-2] += - dIdT(j)*ne*ion.solution[j] +dRdT(j+1)*ne*ion.solution[j+1] -dRdT(j)*ne*ion.solution[j]
                #print(A[off,j])
                #A[j,N-2] = 10
            elif j==ion.Z:
                A[off+j, N-3] += ne*(dIdne(j-1)*ion.solution[j-1] - dIdne(j)*ion.solution[j] - dRdne(j)*ion.solution[j]) + I(j-1)*ion.solution[j-1] - I(j) * ion.solution[j] - R(j)*ion.solution[j]
                A[off+j, N-2] += dIdT(j-1)*ne*ion.solution[j-1] -dIdT(j)*ne*ion.solution[j] - dRdne(j)*ne*ion.solution[j]
            ...
            A[N-2, off+j] += braams_conductivity_derivative_with_respect_to_Z(nfree, T=Te, Z=Z) * (-x**2 * Z * (j**2)/2) 
            if j == 0 :
                A[N-2, off+j] += Di /(dist**2) * (Te-0.025)
            
            A[N-2, N-3] -= ion.solution[j] *( dL_linedne(j) + dL_freedne(j)) #+ ion.IonThreshold[j] * (dIdne(j) - dRdne(j)))
            A[N-2, N-2] -= ion.solution[j] * (dL_linedT(j) + dL_freedT(j)) #+ ion.IonThreshold[j] *(dIdT(j) -dRdT(j)))
            if j != ion.Z:
                A[N-2, N-3] -= e*ion.IonThreshold[j]*(dIdne(j) - dRdne(j+1))
                A[N-2, N-2] -= e*ion.IonThreshold[j]*(dIdT(j) - dRdT(j+1))
# Add fast-electron impact ionization
    ...
    #print(dRdne(1))
    A[N-2, N-3] = A[N-2, N-3] * nfree + ((e**4 *NRE) / (4*pi * eps0**2 *m_e *c) )* getCoulombLogarithm(Te, nfree) \
    ...
    #print(dIdT(0))
    A[N-2, N-2] = A[N-2, N-2] * nfree + e*c*NRE*dEc + dsigma * Ec**2 + sigma * dEc2 - transp_deriv 
    sum1 = sum1* nfree
    offset=0
    for ion in ions:
        for j in range(ion.Z+1):
...
def construct_F(ions: IonHandler, ne, Te, Z, pn, fre=None):
    N = ions.getNumberOfStates()+3
    F = np.zeros((N,))
    Ec = getEc(Te, nfree)
    e = scipy.constants.e
    ...
    for ion in ions:
        ...
        for j in range(ion.Z+1):
            ...
            F[N-3] -= j*ion.solution[j] 
        off += ion.Z+1
        #F[N-1] -= ion.solution[0]
    #print(F)
    return F


A = construct_matrix(ions, ne, Te, Z)
F = construct_F(ions, ne, Te, Z, pn)
